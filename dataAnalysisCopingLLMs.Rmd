  ...
  for(c in conditions){
    for(p in perspectives){
      ...
      temp_g <- plot(conditional_effects(temp_m), plot=FALSE)$`model:forced_decision` + 
        ggtitle(paste0(str_to_title(c),'-',str_to_title(p))) + ylim(ylabs[paste0(bel,'.',c,'1')], ylabs[paste0(bel,'.',c,'2')])
      temp_dat <- subset(dat, condition==c & perspective == p)
      temp_g <- temp_g + geom_point(data=temp_dat, aes_string(x='model', y=paste0(bel,'_2'), color='forced_decision'), shape = 13, size = 2, inherit.aes = FALSE, position = position_jitter(h=0.2,w=0.2))
